...
###########################################################
class EMB_FEAT_LSTM(nn.Module):
	def __init__(self,emb_size,feat_size,voc_size,lstm_hidden_size,f0_dim,linear_h1):
		super(EMB_FEAT_LSTM, self).__init__()
	...
	def forward(self,sents,feat,sent_length):
...
def Train(train_emb,train_feat,train_f0,train_len,val_emb,val_feat,val_f0,val_len,\
	...
	for epoch in range(epoch_num):
		...
		for i in range(len(train_emb)):
			...
			optimizer.zero_grad()
			outputs = model(train_emb_batch,train_feat_batch,train_len_batch)
			# print(outputs.size())
		...
		if (epoch+1)%1==0:
			...
			val_loss,result = Validate(model,val_emb,val_feat,val_f0,val_len)
...
def Validate(model,val_emb,val_feat,val_f0,val_len,save_prediction=""):
	model.eval()
