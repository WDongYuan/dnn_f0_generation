...
class PHRASE_LSTM(nn.Module):
	def __init__(self,emb_size,pos_emb_size,tone_emb_size,
		...
		self.feat_lstm = nn.LSTM(self.emb_size+self.feat_size+self.pos_emb_length*self.pos_emb_size+self.pos_feat_num,self.lstm_hidden_size,
			num_layers=self.lstm_layer,bidirectional=self.bidirectional_flag,batch_first=True)
...
class PHRASE_MEAN_LSTM(nn.Module):
	def __init__(self,emb_size,pos_emb_size,tone_emb_size,
		...
		self.conv_l1 = nn.Linear(feat_size,self.linear_h1)
		self.linear_init(self.conv_l1)
		self.conv_l2 = nn.Linear(self.linear_h1,self.f0_dim)
		self.linear_init(self.conv_l2)

		self.dep_lemb = nn.Linear(self.dep_num,self.dep_lemb_size,bias=False)
		self.linear_init(self.dep_lemb)
	...
	def forward(self,sents,pos,pos_feat,cons,vowel,pretone,tone,postone,feat,phrase,dep,sent_length):
		...
		emb_h_n, (emb_h_t,emb_c_t) = self.emb_lstm(emb_h_0)
		emb_h_t = emb_h_t.permute(1,0,2).contiguous().view(self.batch_size,1,50*self.direction)
		emb_h_t = emb_h_t.expand(self.batch_size,self.max_length,50*self.direction)

		feat_h_0 = torch.cat((feat,emb,pos,pos_feat,emb_h_t),dim=2)
